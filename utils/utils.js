    ...
    if (/[0-9]/g.test(word[i])) {
      for (let k = 0; k < word[i].length; k++) {
        ...
        if (/[0-9]/g.test(temp.charAt(k))) {
          if (
            temp.charAt(k) == "1" &&
            (temp.charAt(k + 1) == "2" || temp.charAt(k + 1) == "3")
          ) {
            let param = `${temp.charAt(k)}${temp.charAt(k + 1)}`;
            word[i] =
              word[i].substring(0, k) +
              mappingNumberToCharacter(param) +
              word[i].substring(k + 2);
          } else {
            word[i] =
              word[i].substring(0, k) +
              mappingNumberToCharacter(word[i].charAt(k)) +
              word[i].substring(k + 1);
          }
        }
      }
    }
...
function removeOnlyOneCharacter(text) {
  return text.replace(/ [a-z]{1,3} /gm, " ");
}
...
function operationMention({ tanggal, tweet, klasifikasi }, code = 1) {
  ...
  let resultRemoveEmote =onlyText(replaceToCharacter);
  let tempTweet2 = ` ${resultRemoveEmote} `;
  if (code == 1) {
  ...
  for (i = 0; i < data.length; i++) {
    ...
    if (code == 1) {
      arrayWord.push(
        new Dataset(
          data[i].tanggal,
          removeExcesSpace(resultWord.trim()),
          data[i].klasifikasi
        )
      );
    } else {
      arrayWord.push(
        new Dataset(
          data[i].tanggal,
          removeExcesSpace(removeOnlyOneCharacter(resultWord.trim())),
          data[i].klasifikasi
        )
      );
    }
